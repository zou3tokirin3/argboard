...
├── ui/
│   ├── index.html
│   ├── main.tsx       # Preact root・2ペインレイアウト・テストフック公開(?test=1時のみ)
│   ├── types.ts       # §5の型定義
...
2ペイン構成。**核導線は「左のストリームからカードを右のボードへドラッグして置き、置いたカード同士に糸を張る」**。
...
| M1     | ザクザク: キャプチャ+ストリーム+db.ts本実装+プロジェクト作成/切替+**エクスポート**                                                                                                                                                                                                                                                                                                     | 3秒で1枚作れる。リロードしても消えない。persist()要求済み。スモーク緑                                                                                    |
| M2     | 繋げる: ボード(パン/ズーム/配置)+糸張り                                                                                                                                                                                                                                                                                                                                                | ストリーム→ボードのドラッグ、糸作成が気持ちよく動く。スモーク緑                                                                                          |
| M3     | 考察: 糸ラベル/矛盾切替/インスペクタ/ボード複数化+**インポート**                                                                                                                                                                                                                                                                                                                       | 考察が言葉として残る。エクスポート→インポートで完全復元。スモーク緑                                                                                      |
