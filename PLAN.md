...
> T015観測は整流済み・done（改善=T016–T020、できたら枠のT021/T022もdone、O2保存信頼はhold＝再観測で昇格）。
...
- T022 = ボード上のカードサイズを変える（T015/O6・できたら・done・改訂25）
...
- **2026-07-28 改訂25(現行)**:
  T022 done。ボードカードを標準/大きめの2段階で切替（`Card.size`・inspector）。
  糸・ヒット・ノリ・サムネは `ui/node-size.ts` から寸法を共有。T015できたら枠は消化済み
