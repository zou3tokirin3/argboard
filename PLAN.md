...
- 自由入力タグはMVP外（T020）。付け／絞りUIは作らない。`tags` フィールドはschema互換のため残すだけ。
  未読・重要などの運用フラグが必要になったら自由タグ復活より専用操作を先に検討する
  - 2026-07-25: T020案Bの発動をT031〜T033として起票（backlog・未採択）。整理用途に限り復活を検討中。
    根拠は「タグ＝描画されないハブカード」（考察カード＋糸と同型で、線を描かず場所を取らない軽量表現）。
    正本の位置は関係層のままなので本節と衝突しない、という読み。T033はT018の起点を
    カード／タグへ広げる案で、採択なら次の改訂番号で§1・§14を書き換える。**採択前に実装へ進めない**
...
- T031 = サイドから自由タグを付ける（サジェスト・種類数抑止つき・T020案Bの発動・未採択）
- T032 = 矩形選択で一括タグ付け／タグの統合・改名（案B制約4の回収手段・未採択）
- T033 = タグを起点にフォーカスビューへ入る（T018の起点拡張・改訂17を伴う・未採択）
