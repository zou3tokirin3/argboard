...
> Won'tリスト」を優先する。**状態: M0〜M3完了。T011最終ドッグフードは省略（dropped）。T010（PWA）done。**
> T008で核体験済み、T009で復元契約済み。M4本線はアプリシェルPWA（オフライン起動＋ホーム画面追加）。
> T015観測は整流済み・done（改善=T016–T020、できたら枠のT021/T022もdone、O2保存信頼はhold＝再観測で昇格）。加えて運用導線の新規候補として T042/T043 を backlog 追加。
...
- 自由入力タグは**整理用途のみ**で復活（T020案B → T031）。主要導線（キャプチャ・配置・糸・考察・
  ...
  未読・重要などの運用フラグには使わない（必要なら専用操作）。一括付与・改名・統合はT032（done）。
  複数選択時の共通タグ一括外しは持たない（1枚選択の × で外す）。
...
- T021 = 探索で出来事を上からも追加する（T015/O3・done・モード分離時点で最上段キャプチャ済み）
- T022 = ボード上のカードサイズを変える（T015/O6・できたら・done・改訂27）
...
- T032 = 矩形選択で一括タグ付け／タグの統合・改名（案B制約4の回収手段・done）
...
- T042 = プロジェクト操作を左上の展開メニューにまとめる（切替/新規/名前変更）
- T043 = 探索モードの追加体験を強化する（ガシガシ追加導線）
- T050 = 探索中の発見元を記録する（`foundVia`・不変・Won't抵触につきパケット待ち）
- T051 = 発見ログを掘った構造で見て畳む（T050の`foundVia`が親・表示層のみ）
- T052 = 発見ログのツリー表示を標準化するか検討する（T051 dogfood 後の判断）
- T053 = 探索モードで発見ログのタイトルをその場で直せるようにする
- T054 = 探索で画像を大きく見ながらメモを書けるようにする（compose 追記時の拡大）
- T055 = 発見ログの絞り込みに発見のみ・考察のみを足す（人間要望「探索のみ／思考のみ」）
- T056 = 考察モードでボード選択時に発見ログを該当行へスクロールする（dropped・T031逆方向は当分見送り）
...
| **M4** | 耐久性: PWA化+必要最小限の磨き | T010 done（シェル precache + 最小インストール案内）。最終ドッグフードT011は省略（核体験T008・復元T009で代替） |
...
- **2026-07-28 改訂23**:
  T032で矩形選択（Shift＋空白ドラッグ）＋紙クリック足し引きによる複数選択と、
  選択集合への一括タグ付与、未定着からの改名・統合を実装（パケット GO・本体 +344）。
  複数選択時の共通タグ一括外しは仕様外。§1・タスク一覧を同期
- **2026-07-28 改訂24**:
  T021を done。探索はヘッダ直下にキャプチャがあり O3（上からも追加）は充足済み。
  追加実装なし（評価パケット案A）。できたら残は T022 のみ
- **2026-07-28 改訂25**:
  T022 実装（標準/大きめ・`Card.size`）。人間確認前に done と誤記したため撤回。
  できたら残は T022（人間確認待ち）のまま
- **2026-07-28 改訂26**:
  人間要望を受けて T042/T043 を backlog 追加。
  T042 はプロジェクト操作を左上展開メニューへ寄せる方針、T043 は探索モードの追加導線強化
- **2026-07-28 改訂27**:
  T022 done（人間確認）。見出し横アイコンで標準/大きめ、大きめ時は画像できるだけ全体表示。
  T015できたら枠は消化済み
- **2026-08-01 改訂28(現行)**:
  人間との設計相談を受けて **T050/T051 を backlog 追加**。**発見の構造（記録・不変・木）と
  意味づけの構造（判断・可変・網＝糸）を別の仕組みにする**方針を採る。発見元は糸ではなく
  `Card.foundVia`（`foundAt` と対になる不変フィールド）に持ち、ボードには描かない
  ＝ボードは意味づけ専用のまま。糸を消しても掘った経路は壊れず、経路を畳んでも糸は無傷。
  当初案（キャプチャ時に自動で糸を張り、糸からツリーを導出する）は、記録と判断を同じ入れ物に
  入れる歪み（「1本目の糸だけが親」という隠れた型付けが必要になる）を理由に取り下げた。
  誤りの訂正は案1＝**外すだけ・付け替え不可**（外し忘れは必ず起きるため外せることは必須）。
  `Card.foundVia` は §12「カードの親子階層」に**抵触する（Yes）**ため、
  評価パケット `packets/T050.md` を提出済み・**人間GO前は実装しない**（§8に論点と代案X/Yを併記）。
  Won't の解除可否・「前作の意味の階層とは別物か」の判定は人間が行う。
  T051（ツリー表示・畳み）は表示層のみでカードに何も足さず、既存のフラット表示は
  置き換えずトグルで併存させる（§1「発見順が基本」を維持）
...
- **OGP・共有プレビュー** — v1 Won't（§12）。Web ARG 向けに将来入れたい意向あり → backlog **T049**（Deno Deploy API。UI は Pages 維持可）
- **共同編集 v2** — §12 Won't 維持。入れるなら第一候補ホストは **Deno Deploy**（WebSocket + CRDT + 保存 API。画像 Blob は外部 object storage 検討）
