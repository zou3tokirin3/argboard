...
- 自由入力タグは**整理用途のみ**で復活（T020案B → T031）。主要導線（キャプチャ・配置・糸・考察・
  ...
  未読・重要などの運用フラグには使わない（必要なら専用操作）。一括・改名・統合はT032、タグ起点の
  視点切替はT033（いずれも未採択）
...
- **フォーカスビュー(T018)**: ボード上の選択カード直上バーから「この視点で見る」。糸で到達するカードだけを濃くし圏外は沈める
  （消さない・触れない）。視点中は同バーで −（一周戻す）＋（もう一周）×（解除）。既定1ホップ。
  状態はsessionのみ（JSONに載せない）。起点は固定のまま、焦点集合内のカードは選び直せる。
  糸は両端とも焦点集合内のときだけ描画。タグ・複数ボード・作中時系列は使わない
...
- T033 = タグを起点にフォーカスビューへ入る（T018の起点拡張・未採択）
