...
> Won'tリスト」を優先する。**状態: M0〜M2完了、M2.5安定化中。**
> 次は連続キャプチャ時の保存競合を直し、糸の意味をラベル一本へ簡素化してから、
> 実ARGドッグフードへ進む。決定の経緯は「14. 決定ログ」参照。
> 末尾「付録A」は完了済みの初期構築手順を履歴として残している。
...
再増額しない。残りは保存修正・インポート・PWAに限定し、超える機能は延期または既存機能と交換する。
...
type Link = {
  ...
  kind: "connects" | "contradicts"; // v1 JSON互換用。UI上の意味はlabel一本にする
  createdAt: number;
};
...
- 糸の意味はラベルを本線とし、「関連 / 矛盾」の種別UI・凡例・色分けは置かない。
  既存JSONの `kind` は読み込むが表示上は通常の糸として扱う
...
| **M1 (done)** | 探索(ザクザク): キャプチャ+発見ログ+プロジェクト作成/切替+エクスポート | 基本導線を人間確認済み。ただし連続入力の保存競合をM2.5で是正する |
| **M2 (done)** | 考察(繋げる): ボード配置+パン/ズーム+糸張り+右インスペクタ+サイド折りたたみ | 配置・糸・往復矢印・ラベル位置を人間確認済み |
| **M2.5** | 安定化と核体験検証: 保存競合修正→糸種別簡素化→過去ARG 45〜60分のドッグフード | 15件以上を記録し、翌日に元資料なしでボードから1つの仮説を説明できる。連続入力で消失しない |
