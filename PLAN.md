...
> 連続キャプチャ時の保存競合はT007で修正済み。次は通常の関係線に後置きできる「要検討」状態、
> 発見／考察カードの最小導線、実ARGドッグフードの順に進む。決定の経緯は「14. 決定ログ」参照。
...
| T013 削除・つながり・見た目 | 60行 | カード削除とインスペクタの糸一覧、発見/考察の形の差。予備とT010枠から移譲 |
| T009 JSONインポート | 100行 | version 1の完全復元と失敗時の非破壊を優先し、補助UIを増やさない |
| T010 PWA | 24行 | manifest・Service Worker・登録処理だけに限定する（枠の一部をT013へ） |
| 予備 | 0行 | T013へ移譲済み |
...
| **M2.5** | 安定化と核体験検証: 保存競合修正→要検討線→考察カード→過去ARG 45〜60分のドッグフード | 15件以上を記録し、要検討線と考察カードを使い分け、翌日に元資料なしでボードから1つの仮説を説明できる。連続入力で消失しない |
