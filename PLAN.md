...
> Won'tリスト」を優先する。**状態: M0〜M2.5完了＋T014 done。次は M3（T009 JSONインポート）。**
> T008ドッグフード済み。ボード移動摩擦はT014で解消。その他観測はT015に退避。
...
- **2026-07-18 改訂8(現行)**:
  ...
  T015観測バックログへ退避し、項目単位で後から昇格する。T014 done。次はM3（T009インポート）
