...
- [ ] 発見ログの絞り込み帯に **発見のみ**（または確定ラベル）と **考察のみ** を追加できる
- [ ] ON のとき `card.role` で一覧が絞れる（省略 role は finding 扱い）
- [ ] 発見のみ／考察のみの排他ルールが決まり、UI と `filteredCards` が一致する
- [ ] 未配置のみ・配置済のみ・検索・タグ視点・ツリーと **AND** で併用できる
- [ ] トグル状態は session のみ（エクスポート JSON に載せない）
- [ ] flow しきい値の見込みを作業ログに書く。超過見込みならパケットへの人間GOがある
- [ ] check / test / smoke が緑
- [ ] 人間が「探索だけ／思考（考察）だけ見たい」が選択メニューで足りると確認する
